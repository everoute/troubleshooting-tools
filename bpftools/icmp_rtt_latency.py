#!/usr/bin/env python2
# -*- coding: utf-8 -*-
# Usage: sudo ./icmp_rtt_latency.py --src-ip <src_ip> --dst-ip <dst_ip> --phy-iface1 <iface1> --phy-iface2 <iface2> [--latency-ms L] [--direction <outgoing|incoming>]
# 
...
def print_event(cpu, data, size):
    ...
    # Check if OVS upcall stages (3 and 4) were hit for Path 1
    ovs_upcall_path1_taken = flow.ts[3] > 0 and flow.ts[4] > 0

    if ovs_upcall_path1_taken:
        _print_latency_segment(2, 3, flow, args.direction)
        _print_latency_segment(3, 4, flow, args.direction)
        _print_latency_segment(4, 5, flow, args.direction)
    else:
        # If OVS upcall path not taken, print the 2->5 "OVS No Upcall" summary 
        # if stage 2 and 5 are present, and 3 and 4 are indeed zero.
        if flow.ts[2] > 0 and flow.ts[3] == 0 and flow.ts[4] == 0 and flow.ts[5] > 0:
    ...
    # Check if OVS upcall stages (10 and 11) were hit for Path 2
    ovs_upcall_path2_taken = flow.ts[10] > 0 and flow.ts[11] > 0

    if ovs_upcall_path2_taken:
        ...
        # If OVS upcall path not taken, print the 9->12 "OVS No Upcall" summary
        # if stage 9 and 12 are present, and 10 and 11 are indeed zero.
        if flow.ts[9] > 0 and flow.ts[10] == 0 and flow.ts[11] == 0 and flow.ts[12] > 0:
...
"""
    ...
    parser.add_argument('--phy-iface2', type=str, required=True,
                      help='Second physical interface to monitor (can be same as iface1 if only one relevant NIC).')
