...
- mem：BPF 程序镜像、JIT 产物、普通 map（ARRAY/HASH/LRU 等）、全局数据段都常驻内核。 占用大小与程序实现一机使用的 map 类型以及架构有关，per-cpu array 最占内存，但是最大占用在程序确定后可以确定。
...
```bash
sudo python2 ebpf-tools/kvm-virt-network/tun/tun_ring_monitor.py --device vnet246
```
...
```bash
sudo python2 ebpf-tools/kvm-virt-network/tun/tun_ring_monitor.py --device vnet246 --all
```
