...
1. **第一层 (Summary)**:
   - 低开销,可在生产环境**持续运行**
   - 建立性能基线,发现异常模式
   - 示例: `system_network_latency_summary.py --interval 60` 全天监控

2. **第二层 (Details)**:
   - 中等开销,**问题复现时启动**
   - 配合过滤器,精确定位到 IP/端口/协议
   - 示例: `vm_network_latency_details.py --src-ip 10.0.0.1 --dst-ip 10.0.0.2`

3. **第三层 (专项)**:
   - 较高开销,**已知瓶颈后短期使用**
   - 深挖根因 (CPU 调度、锁竞争、队列溢出)
   - 示例: `offcputime-ts.py -p <ovs-pid> --duration 60`
