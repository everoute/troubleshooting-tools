...
eBPF-based TCP RTT, in-flight packets, and cwnd histogram collector from SEND perspective.
Captures SRTT, packets_out, snd_cwnd at the moment each data packet is SENT.

Key difference from tcp_rtt_inflight_hist.py:
- This tool probes the SEND path (tcp_rate_skb_sent)
- Sample count = number of packets SENT (denominator for throughput analysis)
- Captures state at transmission time, not ACK reception time
...
Usage examples (root required):
    sudo python tcp_send_rtt_inflight_hist.py --interval 1 --duration 60
    sudo python tcp_send_rtt_inflight_hist.py --laddr 70.0.0.31 --raddr 70.0.0.32
    sudo python tcp_send_rtt_inflight_hist.py --bw-hist --interval 1
    sudo python tcp_send_rtt_inflight_hist.py --lport 5201 --raw

Notes:
- Attaches to tcp_rate_skb_sent() kprobe (SEND perspective, called per packet sent)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="TCP RTT, in-flight, cwnd histogram collector (send perspective)"
    )
...
// Probe tcp_rate_skb_sent - called after each TCP skb is transmitted
// void tcp_rate_skb_sent(struct sock *sk, struct sk_buff *skb)
int kprobe__tcp_rate_skb_sent(struct pt_regs *ctx, struct sock *sk, struct sk_buff *skb) {
    if (!pass_filter(sk))
        return 0;
    ...
    // Read packets_out (in-flight packets, already incremented for this packet)
    u32 packets_out = 0;
    ...
    // Skip if no valid RTT data (connection just started)
    if (srtt_us == 0)
        return 0;
...
def main():
    ...
    print("tcp_send_rtt_inflight_hist started (SEND perspective)")
    print("Probe point: tcp_rate_skb_sent (called per packet sent)")
    ...
    try:
        while True:
            ...
            if now >= next_print:
                ...
                if args.raw:
                    print("\n# interval={} ts={} elapsed={:.1f}s pkts_sent={}".format(
                    ...
                    print("\n==== Interval {} @ {} (elapsed {:.1f}s, pkts_sent: {}) ====".format(
                        interval_num, ts_str, elapsed, cnt))

                    print("\n[RTT (us) at send time] mean={:.1f} p50={} p90={} p99={}".format(
                        rtt_stats["mean"], rtt_stats["p50"], rtt_stats["p90"], rtt_stats["p99"]))
                    rtt_hist.print_log2_hist("usecs")

                    print("\n[In-flight (packets) at send time] mean={:.1f} p50={} p90={} p99={}".format(
                        inflight_stats["mean"], inflight_stats["p50"], inflight_stats["p90"], inflight_stats["p99"]))
                    inflight_hist.print_log2_hist("packets")

                    print("\n[CWND (packets) at send time] mean={:.1f} p50={} p90={} p99={}".format(
                        cwnd_stats["mean"], cwnd_stats["p50"], cwnd_stats["p90"], cwnd_stats["p99"]))
    ...
    ts_str = time.strftime('%Y-%m-%d %H:%M:%S')
    print("\n" + "=" * 70)
    print("CUMULATIVE TOTALS @ {} (SEND perspective)".format(ts_str))
    print("=" * 70)
    ...
    print("\nTotal duration: {:.1f}s, Total packets sent: {}".format(elapsed_total, total_samples))
    if elapsed_total > 0:
        print("Average send rate: {:.0f} pkts/s".format(total_samples / elapsed_total))
